...
from devices_new.rsa_device.RSA_API import *
# from RSA_API import *
...
def create_frequency_array(specSet):
	...
	#print(f"DEBUG: actualStartFreq={specSet.actualStartFreq}, "f"actualFreqStepSize={specSet.actualFreqStepSize}, "f"traceLength={specSet.traceLength}")

	# Create array of frequency data for plotting the spectrum.

	# Проверка на нулевой шаг частоты
	if specSet.actualFreqStepSize == 0:
		# Если шаг нулевой, возвращаем массив с одной частотой
		freq = np.array([specSet.actualStartFreq])
		print(f"WARNING: Zero frequency step size detected. Using single frequency: {specSet.actualStartFreq}")
		return freq
...
def spectrum_example():
	...
	start = 9e3
	stop = 6.2e9
	specSet = config_spectrum(cf, refLevel, span, rbw, start, stop)
