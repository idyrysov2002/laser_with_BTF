...
from config import CURRENTS, DELAYS
from config import NUMBER_RF_MEASURE,ODL_COM, MAIN_SAVE_PATH
...
def build_maps(data_map, save_folder_structure):
    """
...
def main():
    ...
    try:
        ...
        params = itertools.product(CURRENTS, DELAYS)
        ...
        # файл страховка. Данные не потеряется, если код сломается
        backup_file = Path(main_folder) / "backup.txt"
        for idx, (current, delay) in enumerate(params, 1):
            
            base_folder_structure = f"current_{current}mA"
            base_filename = f'delay_{delay}ps_current_{current}mA'
            png_title_point=f'I={current}mA, t={delay}ps'
            maps_folder_structure = f'{main_folder}/maps'
            ...
            with open(backup_file, 'a') as f:
                f.write(f"{current},{delay},{pm_power},{rf_peak_freq_6200MHz/GIGA},{rf_peak_freq_100MHz/GIGA},{rf_peak_freq_1MHz/GIGA},{osc_mean_freq}\n")
